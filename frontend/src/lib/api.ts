import type { AdminStats, Book, BookCollection, BookSearch, DnaProfile, LibraryStatus, Profile, RecommendationBook, User } from '../types'
...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok) { const body = await response.json().catch(() => ({})); throw new ApiError(body.detail || 'Something went wrong', response.status) }
...
export const api = {
  ...
  deleteAccount: (password:string) => request<{ok:boolean}>('/auth/account', { method:'DELETE', body:JSON.stringify({password}) }),
